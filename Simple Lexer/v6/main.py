...
def set_var(name,val):
    global my_vars
    my_vars[name] = Parser().parse(Lexer().lex(val)).calculate().token
class Token:
...
class ParsedItem:
    ...
    def calc(self,tk1,tk2,method,allowed,typ):
        ...
        raise Exception(f"Unsupported operand {self.operand(method)} for type: {typ}")
    def tk_type(self,tk):
        if type(tk) is int:
            return 'INT'
        if type(tk) is float:
    ...
    def calculate(self):
        if len(self.__list) == 1:
            if self.__list[0].type == 'IDENTIFIER':
                return Token(self.get_var(self.__list[0].token),'UNKNOWN',0,0)
